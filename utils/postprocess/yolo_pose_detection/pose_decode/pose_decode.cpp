...
void yolov5_pose_decode_feat(
    const float *const anchors, const uint32_t na, const float stride, const float conf_thres, const uint32_t max_boxes,
    const float *const feat, const uint32_t batch_size,
    const uint32_t ny, const uint32_t nx, const uint32_t no, const uint32_t npose,
    float *const out_batch, uint32_t *out_batch_pos)
{       
    const uint32_t output_params_per_box = 6 + npose * 3;
    const uint32_t max_out_batch_pos = output_params_per_box * max_boxes;
        ...
        for(uint32_t a = 0; a < na; a++){
            ...
            for(uint32_t y = 0; y < ny; y++){
                for(uint32_t x = 0; x < nx; x++){
                    ...
                    if (feat_cur[4] >= conf_thres_logit){

                        const float conf = sigmoid(feat_cur[5]) * sigmoid(feat_cur[4]);
                        if (conf > conf_thres)
                        {
                            assertm(out_pos + output_params_per_box <= max_out_batch_pos, "Reached max number of poses");
                            ...
                            const float bx1 = bw - bw_half;
                            const float bx2 = bw + bw_half;
                            const float by1 = bh - bh_half;
                            const float by2 = bh + bh_half;
                            ...
                            for (uint32_t k = 0; k < npose; k++){
                                ...
                                float kconf = sigmoid(feat_cur[kidx + 2]);
...
void yolov5_pose_decode_feat(
    ...
    const uint32_t lm_count = npose;
    const uint32_t params_per_lm = 3;
    const uint32_t params_per_box = 6 + lm_count * params_per_lm;
    ...
    for (uint32_t b = 0; b < batch_size; b++) {
        ...
        for (uint32_t a = 0; a < num_anchors; a++) {
            ...
            for (uint32_t y = 0; y < ny; y++) {
                for (uint32_t x = 0; x < nx; x++) {
                    const float* const cell = &feat[pos];

                    const float conf_obj_logit = cell[4];

                    // early stopping
                    if (conf_obj_logit >= conf_thres_logit) {
                        const float conf_cls_logit = cell[5];
                        const float conf = sigmoid(conf_obj_logit) * sigmoid(conf_cls_logit);

                        if (conf >= conf_thres) {
                            assertm(out_pos + params_per_box <= max_out_batch_pos, "Reached max number of boxes");

                            float bx = sigmoid(cell[0]);
                            float by = sigmoid(cell[1]);
                            float bw = sigmoid(cell[2]);
                            float bh = sigmoid(cell[3]);
                            ...
                            out[out_pos + 3] = by2;
                            out[out_pos + 4] = conf;
                            out[out_pos + 5] = 0;  // cls is always 0 for person

                            // lm
                            for (uint32_t k = 0; k < lm_count; k++) {
                                const uint32_t kidx = 6 + k*3;
                                float kx = cell[kidx + 0];
                                float ky = cell[kidx + 1];
                                float kconf = cell[kidx + 2];
