...
def yolov8_box_decode(stride, conf_thres, reg_max, feats_box, feats_cls, feats_extra=None):
...
def _yolov5_box_decode_feat(anchors, stride, conf_thres, max_boxes, feat, out_batch, out_batch_pos):
    ...
    if isinstance(feat, QuantizedTensor):
        _clib.box_decode_feat_quant(
            anchors.reshape(-1),
            na,
            stride,
            conf_thres,
...
def yolov5_box_decode(anchors, stride, conf_thres, feats):
    bs = feats[0].shape[0]
    max_boxes = int(1e4)
