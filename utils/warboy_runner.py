...
class WarboyRunner:
    ...
    async def runner(self, input_queue: MpQueue, output_queues: List[MpQueue]):
        async with runtime.create_queue(
            ...
            submit_task = asyncio.create_task(self.submit_with(submitter, input_queue, len_videos))
            recv_task = asyncio.create_task(self.recv_with(receiver, output_queues))
            await submit_task
            await recv_task

    async def submit_with(self, submitter, input_queue, len_videos):
        idx = 0
        while True:
            try:
                input_, contexts, img_idx, video_idx = input_queue[idx%len_videos].get()
