...
def _init():
    ...
    _clib.get_tracked_tracks_count.argtypes = [
        vp,
    ]
    _clib.get_tracked_tracks_count.restype = u32

    _clib.get_lost_tracks_count.argtypes = [
        vp,
    ]
    _clib.get_lost_tracks_count.restype = u32

    _clib.get_removed_tracks_count.argtypes = [
        vp,
    ]
    _clib.get_removed_tracks_count.restype = u32

    _clib.clear_buffer.argtypes = [
        vp,
    ]
    _clib.clear_buffer.restype = None

    _clib.clear_lost.argtypes = [
        vp,
    ]
...
class CByteTrack:
    def __init__(self, track_thresh=0.5, track_buffer=30, match_thresh=0.8, high_thresh=None, framerate=30) -> None:
        if high_thresh is None:
            high_thresh = track_thresh
        self.obj = _clib.ByteTrackNew(track_thresh, high_thresh, match_thresh, framerate, track_buffer)
        self.buffer = None
    ...
    def update(self, boxes):
        ...
        num_tracks = _clib.ByteTrackUpdate(self.obj, np.ascontiguousarray(boxes), num_boxes, self.buffer, n_extra)
