...
def yolov8_box_decode(stride: np.ndarray, conf_thres: float, reg_max: int, feats_box:np.ndarray, feats_cls:np.ndarray, feats_extra: Union[np.ndarray, None] = None) -> List[np.ndarray]:
...
def yolov5_box_decode(anchors: np.ndarray, stride: np.ndarray, conf_thres:float, feats:np.ndarray) -> List[np.ndarray]:
    bs = feats[0].shape[0]
    max_boxes = int(1e4)
