...
class AppRunner:
    ...
    def __init__(self, param, result_queues: List[MpQueue]) -> None:
        ...
        self.frame_queues = [
            MpQueue(QUEUE_SIZE) for _ in range(num_videos)
        ]
        self.output_queues = [
            [MpQueue(QUEUE_SIZE) for _ in range(num_task)] for _ in range(num_videos)
        ]
        ...
        # Warboy Runtime
        self.job_handler = Handler(self.input_queues, self.output_queues, result_queues, param)
    ...
    def _get_warboy_runtime(self, param, num_task, runtime_type="runner"):
        if runtime_type == "queue":
            warboy_runtimes = [WarboyRuntimeQueue(param["model_path"][i], param["worker_num"], param["warboy_device"][i], i) for i in range(num_task)]
        elif runtime_type == "runner":
            warboy_runtimes = [WarboyRuntimeRunner(param["model_path"][i], param["worker_num"], param["warboy_device"][i]) for i in range(num_task)]
        else:
...
class WARBOY_APP:
    ...
    def __init__(self, params):
        ...
        for param in self.params:
            result_queues = [manager.Queue(QUEUE_SIZE) for _ in range(len(param["videos_info"]))]
