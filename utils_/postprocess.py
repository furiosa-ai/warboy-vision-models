...
def draw_bbox(img: np.ndarray, predictions: np.ndarray, class_names: List[str]) -> np.ndarray:
...
def plot_onx_box(box: List[int], img: np.ndarray, color: Tuple[int,int,int], label: str, line_thickness: int = None) -> np.ndarray:
...
def draw_instance_mask(img: np.ndarray, masks: np.ndarray, bbox:np.ndarray, class_names: List[str], alpha:float = 0.5, beta: float=0.5) -> np.ndarray:
    ## Draw instance mask
