...
class ImageMerger:
    ...
    '''
    def __call__(self, output_img_paths: List[str], full_grid_shape: Tuple[int, int] = (1080, 1920)):
        ...
        while ending_channel < num_channel:
            ...
            while c_idx < num_channel:
                grid_img_path = os.path.join(output_img_paths[c_idx], "%010d.bmp" % img_idx)
                ...
                if os.path.exists(grid_img_path):
                    ...
                    ending_signal = os.path.join(output_img_paths[c_idx], "%010d.csv" % img_idx)
...
def make_full_grid(grid_imgs: List[np.ndarray], num_grid: int, grid_shape: Tuple[int,int], full_grid_shape: Tuple[int,int]) -> np.ndarray:
    pad = int(full_grid_shape[0] * 0.1)
    full_grid = np.zeros((full_grid_shape[0]+pad, full))
    ...
    for i, grid_img in enumerate(grid_imgs):
        ...
        full_grid[x0:x1, y0:y1] = grid

    return full_grid

def put_warboy_info(img: np.ndarray, power_info: str, util_info: str, img_shape: Tuple[int,int]):
