...
class InputHandler(JobHandler):
    ...
    '''
    def __init__(self, input_video_paths: List[str], output_dir: str, input_queue: MpQueue, preprocessor, input_shape: Tuple[int,int] = (640, 640)):
        self.video_handlers = [
            mp.Process(target=self.video_to_input, args=(input_video_path, video_idx, input_queue))
    ...
    def video_to_input(self, input_video_path: str, video_idx: int, input_queue: MpQueue) -> None:
...
class OutputHandler(JobHandler):
    ...
    '''
    def __init__(self, input_video_paths: List[str], output_dir: str, output_queues: List[MpQueue], postprocessor, draw_fps:bool = True):
